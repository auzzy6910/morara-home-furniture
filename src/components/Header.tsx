...
export default function Header() {
  ...
  return (
    <header className="bg-white shadow-md sticky top-0 z-50">
      ...
      {/* Top bar */}
      <div className="bg-orange-500 text-white text-sm py-1.5">
        <div className="max-w-7xl mx-auto px-4 flex justify-between items-center">
          ...
          <span className="text-orange-100">Mon - Sat: 8AM - 6PM</span>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-3">
        <div className="flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2">
            <div className="bg-orange-500 text-white font-bold text-xl px-3 py-1.5 rounded">M</div>
            <div>
              <h1 className="text-xl font-bold text-gray-900 leading-tight" style={{ fontFamily: "'Playfair Display', serif" }}>
                Morara Home
              </h1>
              <p className="text-xs text-orange-500 font-medium -mt-0.5">FURNITURE</p>
            </div>
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-8">
            ...
            <Link to="/shop" className="text-gray-700 hover:text-orange-500 font-medium transition-colors">Shop</Link>
            ...
            <SignedOut>
              <div className="hidden md:flex items-center gap-4">
                <SignInButton mode="modal">
                  <button className="text-sm font-medium text-gray-700 hover:text-orange-500 transition-colors">Log In</button>
                </SignInButton>
                <SignUpButton mode="modal">
                  <button className="bg-orange-500 hover:bg-orange-600 text-white rounded-full font-medium text-sm px-4 py-2 transition-colors">
                    Sign Up
                  </button>
                </SignUpButton>
              </div>
        ...
        {mobileMenuOpen && (
          <nav className="md:hidden mt-3 pb-3 border-t pt-3 flex flex-col gap-3">
            <Link to="/" onClick={() => setMobileMenuOpen(false)} className="text-gray-700 hover:text-orange-500 font-medium">Home</Link>
            <Link to="/shop" onClick={() => setMobileMenuOpen(false)} className="text-gray-700 hover:text-orange-500 font-medium">Shop</Link>
            <Link to="/about" onClick={() => setMobileMenuOpen(false)} className="text-gray-700 hover:text-orange-500 font-medium">About</Link>
            <Link to="/contact" onClick={() => setMobileMenuOpen(false)} className="text-gray-700 hover:text-orange-500 font-medium">Contact</Link>
            <SignedOut>
              <div className="flex flex-col gap-2 mt-2">
                <SignInButton mode="modal">
                  <button className="text-gray-700 hover:text-orange-500 font-medium text-left">Log In</button>
                </SignInButton>
                <SignUpButton mode="modal">
                  <button className="bg-orange-500 hover:bg-orange-600 text-white rounded font-medium text-center py-2 transition-colors">
                    Sign Up
                  </button>
                </SignUpButton>
              </div>
